...
import korlibs.image.font.DefaultTtfFontAsBitmap
import korlibs.image.text.RichTextData
import korlibs.image.text.TextAlignment
import korlibs.io.async.Signal
import korlibs.korge.animate.animator
import korlibs.korge.input.*
import korlibs.korge.ui.UIFocusableView
import korlibs.korge.ui.UI_DEFAULT_SIZE
import korlibs.korge.ui.focused
...
inline fun Container.customUiButton(
    label: String = "",
    size: Size = CustomUIButton.DEFAULT_SIZE,
    icon: BmpSlice? = null,
    block: @ViewDslMarker CustomUIButton.() -> Unit = {}
): CustomUIButton = CustomUIButton(size, label, icon).addTo(this).apply(block)
...
) : CustomUIButton(size, text, icon, richText) {
    var pressed: Boolean = false
}

open class CustomUIButton(
    ...
    @ViewProperty
    var bgColorOut = ColorPalette.hover
        set(value) {
            field = value
//            background.bgColor = value
        }
    @ViewProperty
    var bgColorOver = ColorPalette.hover
        set(value) {
            field = value
            updatedUIButton(immediate = true)
        }
    @ViewProperty
    var bgColorSelected = ColorPalette.hover
        set(value) {
            field = value
            updatedUIButton(immediate = true)
        }
...
//                button.animator.tween(background::bgColor[bgcolor], time = 0.25.seconds)
    ...
    init {
        keys {
            down(Key.SPACE, Key.RETURN) { if (this@CustomUIButton.focused) simulateDown(0.5f, 0.5f) }
...
fun <T : CustomUIButton> T.clicked(block: (T) -> Unit): T {
    onClick { block(this) }
    return this
}
